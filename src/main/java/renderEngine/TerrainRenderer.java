...
/**
 * TODO can refactor terrainRenderer and EntityRenderer using Visitor, as there
 * is only MTLLibrary parsing changing. if there are other things changing do
 * visitor for those points too.
 * 
 * @author chezmoi
 *
 */
public class TerrainRenderer {
	...
	public void render(List<Model3D> terrains) {
		for (Model3D terrain : terrains) {
			...
			loadTerrain(terrain);
			GL11.glDrawElements(GL11.GL_TRIANGLES, MasterRenderer
					.storeDataInIntBuffer(terrain.getObjUtils().getOBJUtils().getIndicesAsPrimitiveArray()));
			unbindTerrain();
		}
	}
