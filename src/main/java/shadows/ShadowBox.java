...
public class ShadowBox {
	 ...
	 *            - the in-game camera.
	 */
	protected ShadowBox(Matrix4f lightViewMatrix, CameraEntity camera) {
	...
	protected void update() {
		...
		toNear.scale(DisplayManager.getNearPlane());
	...
	private void calculateWidthsAndHeights() {
		farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(DisplayManager.getFOV())));
		nearWidth = (float) (DisplayManager.getNearPlane()
				* Math.tan(Math.toRadians(DisplayManager.getFOV())));
		farHeight = farWidth / DisplayManager.ASPECT_RATIO;
		nearHeight = nearWidth / DisplayManager.ASPECT_RATIO;
	}
}
